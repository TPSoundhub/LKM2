# M2Sc-Tonegen-basic-stereo.py
#
# Programme that generates tones from sine waves - A basic version with limited amout of code lines for generating a sound
# It includes a bit more compared to the "tonegen basic mono.py" as a step up with regard to functionalities. Taht is:
...
#       - inclusion of time library so it is possible to pause between sounds beeing played back.
#       - added a function to generate a modulated signal 
#
# Revision 0.2 - 30Mar2020 - Knud Funch, Soundhub danmark - LYDKit til undervisningbrug - Region MidtJylland
# To be used as mateial for test in Physics/Math class
#
...
sound_1 = generate_pure_tone(440,8000,True)        # True for l/r in phase
sound_2 = generate_pure_tone(440,8000, False)      # False for l/r out of phase
sound_3 = generate_freq_modulated_tone(440,8000,3) # 3 as modulator frequence - can that be heard?
                                                   # how does it compare to adding two sines with only 3 Hz in difference ?
# etc.... combine sines into more complex sounds - listen and measure ... as in "tonegen basic mono.py"

sound_4 = generate_pure_tone(443,8000,True)

sound_6 = sound_1+sound_4  # try this with sound 4 out of phase compare to in phase 


#play_sound(pygame.sndarray.make_sound(sound_1),1,1,False)  # Use this to make an experiment with one/two speakers and measure intensity


#play_sound(pygame.sndarray.make_sound(sound_2),1,1,False)  # Why can we hear this as the 2 signals added is zero?
#play_sound(pygame.sndarray.make_sound(sound_3),1,0,False)
#play_sound(pygame.sndarray.make_sound(sound_3),0,1,False)  

# use True instead of False as parameter to make sound play 'forever' - that is within Thonny until you press stop

#play_sound(pygame.sndarray.make_sound(sound_6),1,1,True)    

play_sound(pygame.sndarray.make_sound(sound_1),0,1,True)   # 440 in right  - try with earphone - what happens - how does it compare to mono beats experiment?
play_sound(pygame.sndarray.make_sound(sound_4),1,0,True)   # 443 in left
